...
ALT_PATTERNS = [
    ...
    (?P<label>[A-Z])              # A/B/C
    \s*[:(]\s*
    (?P<body>[^)|;]+)             # impact 9, cost 7000, risk 0.25
    [);]?
    """,
    r"""
...
# Hyväksyy: key=9, key:9, key 9, key = 9, key : 9
KV_PAT = re.compile(
    r"""
    (?P<key>[a-zA-Z_]+)          # avain
    \s*[:=]?\s*                  # valinnainen : tai =
    (?P<val>-?(?:\d+(?:\.\d+)?|\.\d+))  # numero (myös .2)
    """,
    ...
    for pat in ALT_PATTERNS:
        for m in re.finditer(pat, text, re.VERBOSE | re.IGNORECASE):
            ...
            for km in KV_PAT.finditer(body):
                ...
                kvs[k] = v
            if kvs:  # lisää vain jos löytyi arvoja
                options[label] = kvs

    # Jos mitään A/B-arvoja ei löytynyt, tarjoa klassinen fallback (A vs B → luo A,B,C)
...
def _decide_attribute_polarity(keys: List[str]) -> Dict[str, str]:
    ...
    for k in keys:
        ...
        if lk in BENEFIT_KEYS:
            ...
            pol[k] = "cost" if any(x in lk for x in ("risk","cost","price","loss")) else "benefit"
    return pol
...
def _normalize_matrix(options: Dict[str, Dict[str, float]], polarity: Dict[str, str]) -> Tuple[Dict[str, Dict[str, float]], Dict[str, Tuple[float, float]]]:
    ...
    for k in all_keys:
        vs = [options[o][k] for o in options if k in options[o]]
        if not vs:
            continue
        lo, hi = _min_max(vs)
        domains[k] = (lo, hi)
...
def _weights_from_ctx(session_ctx: Dict[str, Any], keys: List[str]) -> Dict[str, float]:
    ...
    if s < 1.0:
        ...
        if zeros:
            ...
            for k in zeros: w[k] += extra
        else:
            factor = 1.0 / s
            for k in keys: w[k] *= factor
    elif s > 1.0:
        factor = 1.0 / s
        for k in keys: w[k] *= factor
    return w
...
def _stability_check(U: Dict[str, float], weights: Dict[str, float], S: Dict[str, Dict[str, float]]) -> Tuple[bool, str]:
    ...
    for k in keys:
        ...
        s = sum(w2.values()); 
        if s <= 0: continue
        for kk in w2: w2[kk] /= s
        U2 = {o: sum(w2.get(kk,0.0)*S[o].get(kk,0.0) for kk in S[o]) for o in S}
...
def run(user_text: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    md: List[str] = []
    md.append("# StrategyMCDA")
    md.append("**Criteria:**")
    for k in all_keys:
        tag = "benefit" if polarity.get(k) == "benefit" else "cost"
        md.append(f"- {k.capitalize()} ({tag})")

    md.append("\n**Weights:**")
    for k in all_keys:
        md.append(f"- {k.capitalize()}: {weights.get(k,0.0):.3f}")

    md.append("\n**Options:**")
    for o, kv in options.items():
        parts = [f"{k.capitalize()}={kv[k]}" for k in all_keys if k in kv]
        md.append(f"- {o}: " + ", ".join(parts))

    md.append("\n**Scores:**")
    for o in sorted(S.keys()):
        parts = [f"{k.capitalize()}:{S[o].get(k,0.0):.3f}" for k in all_keys if k in S[o]]
        md.append(f"- {o}: U={U.get(o,0.0):.3f}  |  [{', '.join(parts)}]")

    md.append("\n**Recommendation:**")
    md.append(f"Choose {rec} (highest utility U)." if rec else "Insufficient data.")
    md.append("\n**Uncertainty:**")
    md.append(note)
